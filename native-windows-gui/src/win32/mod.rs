...
pub fn init_common_controls() -> Result<(), NwgError> {
    ...
    use winapi::um::commctrl::{ICC_BAR_CLASSES, ICC_STANDARD_CLASSES, ICC_DATE_CLASSES, ICC_PROGRESS_CLASS,
     ICC_TAB_CLASSES, ICC_TREEVIEW_CLASSES, ICC_LISTVIEW_CLASSES, ICC_COOL_CLASSES, ICC_LINK_CLASS, ICC_ANIMATE_CLASS, ICC_USEREX_CLASSES, ICC_HOTKEY_CLASS, ICC_PAGESCROLLER_CLASS};
    ...
    unsafe {
        ...
        if cfg!(feature = "syslink") {
            classes |= ICC_LINK_CLASS;
        }

        if cfg!(feature = "animation") {
            classes |= ICC_ANIMATE_CLASS;
        }
        ...
        if cfg!(feature = "pager") {
            classes |= ICC_PAGESCROLLER_CLASS;
        }
